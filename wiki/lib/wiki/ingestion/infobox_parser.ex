defmodule Wiki.Ingestion.InfoboxParser do
  @moduledoc """
  ...
  ## Example
      ...
      ...> }}
      ...> \"""
      iex> InfoboxParser.parse(wikitext)
      {:ok, %{
        "infobox_type" => "Item",
        "name" => "Abyssal whip",
        "members" => "Yes",
  ...
  Useful when you only need certain values.
  """
  ...
  # Extract a balanced {{ ... }} template starting at position
  ...
  # Parse infobox template into a map
  defp parse_infobox(template) do
    # Extract infobox type from {{Infobox Type
    type =
    ...
    # Strip outer braces and Infobox Type header
    content =
      template
  ...
  # Split by | but not inside nested templates
  ...
  # Remove common wiki markup from values
