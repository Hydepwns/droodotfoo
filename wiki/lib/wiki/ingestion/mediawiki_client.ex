defmodule Wiki.Ingestion.MediaWikiClient do
  @moduledoc """
  ...
  Returns the page content including rendered HTML and optionally wikitext.
  """
  ...
  More efficient than individual get_page calls for bulk operations.
  Returns a map of title => page data.
  """
  @spec get_pages([String.t()], keyword()) :: {:ok, %{String.t() => page()}} | {:error, term()}
  def get_pages(titles, opts \\ []) when is_list(titles) do
    # MediaWiki API limits to 50 titles per request
    titles
    |> Enum.chunk_every(50)
  ...
  Returns pages modified after the given timestamp.
