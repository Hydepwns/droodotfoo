defmodule WikiWeb.Parts.IndexLive do
  ...
  def handle_event("search", %{"search" => %{"q" => query}}, socket) do
    ...
    {:noreply, push_patch(socket, to: ~p"/parts?#{params}")}
  end
    ...
    <Layouts.app flash={@flash}>
      <div class="max-w-6xl mx-auto px-4 py-8">
        <header class="flex items-center justify-between mb-8">
          ...
          <.link navigate={~p"/parts/add"} class="btn-primary">
            + Add Part
          </.link>
        </header>
        ...
        <div :if={@parts == []} class="text-center py-12 text-zinc-500 font-mono">
          No parts found.
          <.link navigate={~p"/parts/add"} class="text-blue-400 hover:underline">
            Add one?
          </.link>
        </div>
  ...
  defp part_card(assigns) do
    ~H"""
    <.link
      navigate={~p"/parts/#{@part.part_number}"}
