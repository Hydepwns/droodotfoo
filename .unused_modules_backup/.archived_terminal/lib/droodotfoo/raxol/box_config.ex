defmodule Droodotfoo.Raxol.BoxConfig do
  ...
  ## Examples
      ...
      iex> BoxConfig.header_line("Games")
      "┌─ Games ─────────────────────────────────────────────────────────────┐"
  ...
  Returns a list of lines where each line fits within the max_width.
  Attempts to break at word boundaries when possible.

  ## Parameters
  - `text` - The text to wrap
  - `max_width` - The maximum width per line

  ## Examples

      iex> BoxConfig.wrap_text("Hello world", 20)
      ["Hello world"]

      iex> BoxConfig.wrap_text("This is a very long line that needs wrapping", 15)
  ...
  defp wrap_single_line("", _max_width), do: [""]

  defp wrap_single_line(line, max_width) do
    words = String.split(line, " ", trim: true)
    wrap_words(words, max_width, "", [])
  end

  # Recursively build wrapped lines from words
  defp wrap_words([], _max_width, "", acc), do: Enum.reverse(acc)
  defp wrap_words([], _max_width, current, acc), do: Enum.reverse([String.trim(current) | acc])

  defp wrap_words([word | rest], max_width, current, acc) do
    ...
    cond do
      ...
      word_len > max_width ->
        # If we have current content, add it first
        new_acc = if current != "", do: [String.trim(current) | acc], else: acc
        # Split the long word
        {word_part, remainder} = String.split_at(word, max_width)
        # Continue with remainder as a new word
        wrap_words([remainder | rest], max_width, "", [word_part | new_acc])

      # If adding this word would exceed max_width, start a new line
      current != "" and String.length(current) + 1 + word_len > max_width ->
        wrap_words(rest, max_width, word, [String.trim(current) | acc])

      # Add word to current line
      current == "" ->
        wrap_words(rest, max_width, word, acc)

      true ->
        wrap_words(rest, max_width, current <> " " <> word, acc)
    end
  end

  @doc """
  Wraps and pads text to create multiple box lines.

  Returns a list of padded lines ready to be inserted into a box.
  ...
  ## Examples
      ...
      iex> BoxConfig.wrap_and_pad("Hello world", 20)
      ["Hello world         "]

  """
  @spec wrap_and_pad(String.t(), pos_integer()) :: [String.t()]
  def wrap_and_pad(text, width) do
    text
    |> wrap_text(width)
  ...
  @doc """
  Wraps text and creates complete padded lines with indent.

  Similar to padded_line but returns multiple lines if text wraps.
  Each line has a 2-space indent automatically added.
  ...
  ## Examples
      ...
      iex> BoxConfig.wrapped_padded_lines("Hello", 20)
      ["  Hello             "]

      iex> BoxConfig.wrapped_padded_lines("This is a very long line", 15)
      ["  This is a    ", "  very long    ", "  line         "]

  """
  @spec wrapped_padded_lines(String.t(), pos_integer()) :: [String.t()]
  def wrapped_padded_lines(text, width \\ @inner_width) do
    # Reserve 2 chars for leading indent
    content_width = width - 2
    ...
    |> Enum.map(fn line ->
      "  #{String.pad_trailing(line, content_width)}"
    end)
  end
