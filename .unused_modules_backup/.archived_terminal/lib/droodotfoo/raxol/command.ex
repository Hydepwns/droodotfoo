defmodule Droodotfoo.Raxol.Command do
  ...
  # alias Droodotfoo.Resume.Samples, as: ResumeSamples  # Module in backup - TODO: restore if needed
  # alias Droodotfoo.Content.Portfolio  # Module in backup - TODO: restore if needed
  ...
  def handle_input(key, state) when byte_size(key) == 1 do
    # Add character to command buffer and update fuzzy suggestions
    new_buffer = state.command_buffer <> key

    state
    |> Map.put(:command_buffer, new_buffer)
    |> update_fuzzy_suggestions()
  end
  ...
  defp navigate_history_up(state) do
    ...
    # Guard against nil command_buffer
    if is_binary(state.command_buffer) && String.starts_with?(state.command_buffer, "search ") do
  ...
  defp select_autocomplete_suggestion(state, suggestions, index) do
    ...
    # Extract command name from suggestion object
    # Guard against nil selection
    command_name =
      cond do
        is_nil(selected) -> ""
        is_map(selected) -> selected.command || ""
        is_binary(selected) -> selected
        true -> ""
      end

    state
    |> Map.put(:command_buffer, command_name)
    |> clear_autocomplete()
  end
  ...
  def execute_terminal_command(command, state) do
    ...
    case CommandParser.parse_and_execute(command, state.terminal_state) do
      ...
      {:ok, output, new_terminal_state} ->
        ...
        {web3_action, cleaned_terminal_state} = Map.pop(temp_state2, :web3_action)
      ...
      {:plugin, plugin_name, output} ->
        ...
        new_output = append_to_output(state.terminal_output, output)
        # Safely convert to existing atom to avoid atom table pollution
        section =
          try do
            String.to_existing_atom(plugin_name)
          rescue
            ArgumentError -> :terminal
          end
  ...
  defp append_to_output(current_output, new_content) when is_binary(new_content) do
  ...
  defp append_to_output(current_output, nil), do: current_output
  defp append_to_output(current_output, _non_string), do: current_output
  ...
  defp run_command({"stl", args_str}, state) do
    ...
    # Guard against nil or non-string args_str
    args = if is_binary(args_str), do: String.split(args_str, " "), else: []

    case StlCommands.execute("stl", args, state) do
      {:ok, _output} -> state
  ...
  defp run_command({"loadresume", _args}, state) do
    # TODO: Restore Resume.Samples module from .unused_modules_backup/samples.ex
    # Load sample resume data or parse JSON from args
    # For now, stub with empty data
    resume_data = %{}
  ...
  defp parse_search_mode(query) when is_binary(query) do
    cond do
  ...
  defp parse_search_mode(nil), do: {nil, ""}
  defp parse_search_mode(_non_string), do: {nil, ""}

  defp get_searchable_content(_state) do
    # TODO: Restore Content.Portfolio module from .unused_modules_backup/portfolio.ex
    # For now, return empty list
    []
  end

  # Update fuzzy search suggestions based on current buffer
  defp update_fuzzy_suggestions(state) do
    alias Droodotfoo.Terminal.CommandRegistry

    # Store full suggestion objects (not just display text)
    # This allows the renderer to show category badges and richer UI
    suggestions =
      if String.length(state.command_buffer) > 0 do
        CommandRegistry.get_suggestions(state.command_buffer, 8)
      else
        []
      end

    %{state | autocomplete_suggestions: suggestions, autocomplete_index: 0}
  end
