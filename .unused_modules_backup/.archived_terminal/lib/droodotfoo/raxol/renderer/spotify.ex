defmodule Droodotfoo.Raxol.Renderer.Spotify do
  ...
  def draw_dashboard do
    ...
    now_playing =
      build_now_playing_section(playback_data.current_track, playback_data.playback, state_icon)
  ...
  defp build_now_playing_section(%{name: name, artists: artists}, playback, state_icon) do
    ...
    truncated_name = BoxConfig.truncate_text(name, 40)
    truncated_artist = BoxConfig.truncate_text(artist_names, 40)
  ...
  def draw_view(state) do
    ...
    if Map.get(state, :spotify_compact_mode, false) do
      draw_compact()
    else
  ...
  @doc """
  Draw a minimal/compact Spotify view.
  """
  def draw_compact do
    [
      "┌─ Spotify ───────────────────────────────────────────────────────────┐",
  ...
  def draw_devices do
    ...
    device_lines =
      if current_device do
        device_name = BoxConfig.truncate_text(current_device["name"] || "Unknown", 40)
        device_type = BoxConfig.truncate_text(current_device["type"] || "Unknown", 15)
    ...
    now_playing =
      case current_track do
        %{name: name, artists: artists} ->
          artist_names = Enum.map_join(artists, ", ", & &1["name"])
          truncated_name = BoxConfig.truncate_text(name, 50)
          truncated_artist = BoxConfig.truncate_text(artist_names, 50)
  ...
  def build_progress_bar(playback) do
    case playback do
      ...
      _ ->
        # No playback data
        BoxConfig.truncate_and_pad("--:-- / --:--", BoxConfig.inner_width())
    end
  end
end
