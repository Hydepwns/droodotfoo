defmodule Droodotfoo.Terminal.Commands.FileOps do
  ...
  @impl true
  def execute("find", args, state), do: find(args, state)
  def execute("cat", args, state), do: cat(args, state)
  def execute("head", args, state), do: head(args, state)
  def execute("tail", args, state), do: tail(args, state)
  def execute("grep", args, state), do: grep(args, state)
  def execute("rm", args, state), do: rm(args, state)
  def execute("touch", args, state), do: touch(args, state)
  def execute("mkdir", args, state), do: mkdir(args, state)
  def execute("cp", args, state), do: cp(args, state)
  def execute("mv", args, state), do: mv(args, state)
  def execute("wc", args, state), do: wc(args, state)

  def execute(command, _args, state) do
    {:error, "Unknown file operation command: #{command}", state}
  end
