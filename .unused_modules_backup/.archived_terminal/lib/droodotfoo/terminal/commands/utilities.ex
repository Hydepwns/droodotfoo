defmodule Droodotfoo.Terminal.Commands.Utilities do
  ...
  @impl true
  def execute("help", args, state), do: help(args, state)
  def execute("?", args, state), do: help(args, state)
  def execute("man", args, state), do: man(args, state)
  def execute("clear", _args, state), do: clear(state)
  def execute("cls", _args, state), do: clear(state)
  def execute("history", _args, state), do: history(state)
  def execute("themes", _args, state), do: themes(state)
  def execute("theme", args, state), do: theme(args, state)
  def execute("perf", args, state), do: perf(args, state)
  def execute("dashboard", args, state), do: dashboard(args, state)
  def execute("metrics", args, state), do: metrics(args, state)
  def execute("crt", args, state), do: crt(args, state)
  def execute("contrast", args, state), do: contrast(args, state)
  def execute("search", args, state), do: search(args, state)
  def execute("/", args, state), do: search(args, state)
  def execute("tree", args, state), do: tree(args, state)

  def execute(command, _args, state) do
    {:error, "Unknown utility command: #{command}", state}
  end
  ...
  # Delegate to dedicated Performance.Commands module

  @doc """
  Performance monitoring and optimization commands.
  """
  def perf(args, state) do
    Droodotfoo.Terminal.Commands.Performance.execute("perf", args, state)
  end

  def dashboard(_args, state) do
    # Open full performance monitor
    Droodotfoo.Terminal.Commands.Performance.execute("perf", ["monitor"], state)
  end

  def metrics(_args, state) do
    # Show performance metrics
    Droodotfoo.Terminal.Commands.Performance.execute("perf", ["metrics"], state)
  end
