defmodule Droodotfoo.Terminal.Commands.Navigation do
  ...
  @impl true
  def execute("ls", args, state), do: ls(args, state)
  def execute("dir", args, state), do: ls(args, state)
  def execute("cd", args, state), do: cd(args, state)
  def execute("pwd", _args, state), do: pwd(state)

  def execute(command, _args, state) do
    {:error, "Unknown navigation command: #{command}", state}
  end
