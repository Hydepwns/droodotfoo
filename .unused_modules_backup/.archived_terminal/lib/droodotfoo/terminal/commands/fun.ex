defmodule Droodotfoo.Terminal.Commands.Fun do
  ...
  @impl true
  def execute("fortune", _args, state), do: fortune(state)
  def execute("cowsay", args, state), do: cowsay(args, state)
  def execute("sl", _args, state), do: sl(state)
  def execute("sudo", args, state), do: sudo(args, state)
  def execute("vim", _args, state), do: vim([], state)
  def execute("vi", _args, state), do: vim([], state)
  def execute("emacs", _args, state), do: emacs([], state)
  def execute("exit", _args, state), do: exit(state)
  def execute("quit", _args, state), do: exit(state)
  def execute("logout", _args, state), do: exit(state)

  def execute(command, _args, state) do
    {:error, "Unknown fun command: #{command}", state}
  end
