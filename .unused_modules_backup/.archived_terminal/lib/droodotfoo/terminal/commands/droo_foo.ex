defmodule Droodotfoo.Terminal.Commands.DrooFoo do
  ...
  alias Droodotfoo.Terminal.Commands.{FileOps, Resume}

  @impl true
  def execute("skills", args, state), do: skills(args, state)
  def execute("resume", args, state), do: resume(args, state)
  def execute("contact", args, state), do: contact(args, state)
  def execute("download", args, state), do: download(args, state)
  def execute("projects", args, state), do: projects(args, state)
  def execute("project", args, state), do: project(args, state)
  def execute("charts", _args, state), do: charts(state)
  def execute("api", args, state), do: api(args, state)
  def execute("resume_export", args, state), do: resume_export(args, state)
  def execute("resume_formats", args, state), do: resume_formats(args, state)
  def execute("resume_preview", args, state), do: resume_preview(args, state)
  def execute("contact_form", args, state), do: contact_form(args, state)
  def execute("contact_status", args, state), do: contact_status(args, state)

  def execute(command, _args, state) do
    {:error, "Unknown droo.foo command: #{command}", state}
  end
  ...
  @doc """
  Display resume from resume.txt file, or handle resume filtering/search subcommands.

  Subcommands handled by Commands.Resume:
  - filter, search, preset, export, technologies, autocomplete, clear
  """
  def resume([], state) do
    # Delegate to FileOps.cat for displaying resume
    FileOps.cat(["resume.txt"], state)
  end

  def resume(args, state) when length(args) > 0 do
    # Delegate to Commands.Resume for filtering/search functionality
    Resume.execute("resume", args, state)
  end
