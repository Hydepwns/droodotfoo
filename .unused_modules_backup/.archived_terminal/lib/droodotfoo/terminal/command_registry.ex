defmodule Droodotfoo.Terminal.CommandRegistry do
  ...
  @commands [
    ...
    # Resume Export & Filtering
    %{
      name: "resume",
      aliases: ["cv"],
      category: :content,
      description: "Interactive resume filtering, search, and export"
    },
    %{
      name: "resume_export",
      aliases: [],
  ...
  Returns list of matching commands with scores, sorted by relevance.
  Each result includes formatted suggestion text with description and aliases.
  """
  def search_commands(query) when is_binary(query) and byte_size(query) > 0 do
    alias Droodotfoo.Terminal.FuzzySearch

    FuzzySearch.search(@commands, query, fn cmd ->
      # Search across name and all aliases
      [cmd.name | cmd.aliases]
    end)
    |> Enum.map(fn {cmd, score} ->
      %{
        command: cmd.name,
        display_text: format_suggestion(cmd),
  ...
  def search_commands(""), do: []

  @doc """
  Get top N command suggestions for display.

  Returns formatted suggestions with descriptions and aliases.
  """
  def get_suggestions(query, limit \\ 5) do
    search_commands(query)
    |> Enum.take(limit)
  end

  @doc """
  Format a command for display in autocomplete dropdown.

  Shows: "command (alias) - description"
  """
  def format_suggestion(cmd) do
    aliases_text =
      case cmd.aliases do
        [] -> ""
    ...
    "#{cmd.name}#{aliases_text} - #{cmd.description}"
  end
