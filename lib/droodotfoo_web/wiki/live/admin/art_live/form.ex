defmodule DroodotfooWeb.Wiki.Admin.ArtLive.Form do
  @moduledoc """
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> assign(current_path: "/admin/art/add")
     |> assign(artwork: nil)
    ...
    case result do
      {:ok, article} ->
        {:noreply,
         ...
         |> push_navigate(to: "/art/#{article.slug}")}
      ...
      <div class="max-w-2xl mx-auto px-4 py-8">
        ...
        <.form for={@form} phx-change="validate" phx-submit="save" class="space-y-6">
          ...
          <div class="flex justify-end gap-4 pt-4">
            <.link navigate="/admin/art" class="btn-secondary">Cancel</.link>
  ...
  # Define a basic input component since we don't have WikiWeb helpers
  ...
  defp input(%{type: "textarea"} = assigns) do
    ~H"""
    <div>
      <label :if={@label} class="block text-sm font-medium text-zinc-300 mb-1">{@label}</label>
      <textarea
        name={@field.name}
        id={@field.id}
        rows={@rows || 3}
        placeholder={@placeholder}
        class="w-full bg-zinc-900 border border-zinc-700 rounded px-3 py-2 font-mono text-sm"
