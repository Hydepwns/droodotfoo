defmodule Droodotfoo.Terminal.Commands.Stl do
  ...
  def execute(["help" | _], state) do
    execute([], state)
  end

  def execute(["load", url | _], state) do
    viewer_state =
  ...
  def execute(["load"], _state) do
    {:error, "stl load: missing URL argument\nUsage: :stl load <url>"}
  end

  def execute(["info" | _], state) do
  ...
  def execute(["mode", mode_str | _], state) do
  ...
  def execute(["rotate", axis_str | _], state) do
  ...
  def execute([unknown | _], _state) do
    {:error, "Unknown STL command: #{unknown}\nType ':stl help' for usage"}
  end
