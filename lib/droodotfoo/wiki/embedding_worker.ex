defmodule Droodotfoo.Wiki.EmbeddingWorker do
  @moduledoc """
  ...
  ## Manual Invocation
      ...
      %{full: true} |> Droodotfoo.Wiki.EmbeddingWorker.new() |> Oban.insert()

      # Re-embed specific source
      %{source: "nlab"} |> Droodotfoo.Wiki.EmbeddingWorker.new() |> Oban.insert()
  ...
  defp process_singles(articles, stats) do
    Enum.reduce(articles, stats, fn article, acc ->
      ...
      case Ollama.embed(text) do
        ...
        {:error, reason} ->
          Logger.warning(
            "EmbeddingWorker: single embed failed for #{article.id}: #{inspect(reason)}"
          )
