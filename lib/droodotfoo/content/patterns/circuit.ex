defmodule Droodotfoo.Content.Patterns.Circuit do
  ...
  defp generate_single_trace(index, width, height, config, palette, rng, animate) do
    ...
    trace_element =
      if animate do
        SVGBuilder.with_class(trace_element, "circuit-glow")
      else
        trace_element
      end
  ...
  def generate_svg(slug, width, height, animate \\ false) do
    ...
    {_palette_name, palette} = PatternConfig.choose_palette_for_style(slug, :circuit)
    {elements, _rng} = generate(width, height, rng, palette, animate)

    animations =
      if animate do
        Droodotfoo.Content.PatternAnimations.get_animations(:circuit)
      else
        ""
      end
