defmodule Droodotfoo.Git.Forgejo do
  @moduledoc """
  Forgejo source adapter for the git browser.

  Uses Forgejo REST API to browse repos, files, and commits.
  """
  ...
  defp base_url do
    Application.get_env(:droodotfoo, :forgejo)[:base_url] || "http://mini-axol:3000"
  end

  defp token do
    Application.get_env(:droodotfoo, :forgejo)[:token]
  end
  ...
  def list_repos(owner, opts \\ []) do
    ...
    case get("/users/#{owner}/repos", %{limit: limit, page: page}) do
  ...
  def get_commits(owner, repo, branch, opts \\ []) do
    ...
    params = %{sha: branch, limit: limit, page: page}
  ...
  defp get(path, params \\ %{}) do
    ...
    headers = if token(), do: [{"authorization", "token #{token()}"} | headers], else: headers
    ...
    case Req.get(request_opts) do
      ...
      {:ok, %Req.Response{status: status, body: body}} ->
        Logger.warning("Forgejo API error: status=#{status} path=#{path}")
        {:error, {:http_error, status, body}}

      {:error, reason} ->
        Logger.warning("Forgejo API request failed: #{inspect(reason)}")
        {:error, reason}
    end
  end
